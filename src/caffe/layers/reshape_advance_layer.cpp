...
//
// Reshap Modification
// Reshape bottom[0] to the shape of bottom[1](as reference).
// Parameter reshape_param().shape().dim() = -2
// ------------------------------------------------------------------

#include <vector>

#include "caffe/layers/reshape_advance_layer.hpp"

namespace caffe {

template <typename Dtype>
void ReshapeAdvanceLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void ReshapeAdvanceLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  for (int i = 0; i < top_num_axes; ++i) {
    ...
    else if (top_dim == -2) //(added) refer to the bottom[1]
   { 
      constant_count_ *= bottom[1]->shape(i);
   }
    else {
  ...
  for (int i = 0; i < num_new_axes; ++i) {
    const int dim = top_blob_shape.dim(i);
    if (dim == -2) {
       top_shape[top_shape_index++] = bottom[1]->shape(i);
    }
    else {
...
INSTANTIATE_CLASS(ReshapeAdvanceLayer);
REGISTER_LAYER_CLASS(ReshapeAdvance);
